...
function isEventEnabled(pixelEvents = {}, eventName = '') {
    const name = String(eventName || '').trim().toLowerCase();
    if (name === 'purchase') return pixelEvents.purchase !== false;
    if (name === 'initiatecheckout') return pixelEvents.checkout !== false;
    if (name === 'addpaymentinfo') return pixelEvents.checkout !== false;
    if (name === 'lead') return pixelEvents.lead !== false;
    if (name === 'pageview') return pixelEvents.page_view !== false;
    return true;
}
...
function buildFbcFromFbclid(fbclid = '') {
    const clean = sanitizeText(fbclid);
    if (!clean) return '';
...
async function sendPixelServerEvent(eventName, payload = {}, req = null) {
    ...
    if (!pixel.enabled || !pixel.id || !capi.enabled || !capi.accessToken || !isEventEnabled(events, eventName)) {
    ...
    const fbp = sanitizeText(payload?.fbp || payload?._fbp || '');
    const fbclid = sanitizeText(payload?.fbclid || payload?.utm?.fbclid || '');
    const fbc = sanitizeText(payload?.fbc || payload?._fbc || '') || buildFbcFromFbclid(fbclid);
    ...
    const sourceUrl = sanitizeText(payload?.sourceUrl || payload?.source_url || '');
    const eventData = {
        ...
        custom_data: {
            ...
            content_name: String(payload?.shippingName || payload?.shipping?.name || ''),
            content_category: payload?.isUpsell ? 'upsell' : 'checkout',
            content_type: 'product'
        }
    };
    if (sourceUrl) eventData.event_source_url = sourceUrl;
    const eventId = String(payload?.eventId || payload?.orderId || payload?.txid || payload?.pixTxid || '').trim();
