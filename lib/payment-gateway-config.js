...
const SUPPORTED_GATEWAYS = new Set(['ativushub', 'ghostspay', 'sunize']);
...
function buildAtivushubConfig(raw = {}) {
    ...
    return {
        ...
        webhookToken: toText(raw.webhookToken) || toText(process.env.ATIVUSHUB_WEBHOOK_TOKEN) || 'dev',
...
function buildGhostspayConfig(raw = {}) {
    ...
    return {
        ...
        webhookToken:
            ...
            toText(process.env.GHOSTSPAY_WEBHOOK_TOKEN) ||
            toText(process.env.ATIVUSHUB_WEBHOOK_TOKEN) ||
            'dev',
...
function buildSunizeConfig(raw = {}) {
    return {
        ...
        webhookToken:
            toText(raw.webhookToken) ||
            toText(process.env.SUNIZE_WEBHOOK_TOKEN) ||
            toText(process.env.ATIVUSHUB_WEBHOOK_TOKEN) ||
            'dev',
...
function buildPaymentsConfig(raw = {}) {
    ...
    if (!enabledByGateway[activeGateway]) {
        activeGateway =
            (['ativushub', 'ghostspay', 'sunize'].find((gatewayId) => enabledByGateway[gatewayId])) ||
